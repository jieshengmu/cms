...
.GM(
		{
			...
			ajaxData : '/system/feedBacks/list', //请求路径 -自动发送的ajax请求
			ajaxType : 'POST',//请求方法
			...
			columnData : [ //列的数据 -- 返回的json数据对应
				...
				},
				{
					...
					template : function(cell, row, index,
						...
						return '<img src="'+cell+'" width="100px" alt="无效图片" />';
					}
				},
				{
					key : 'createDate',
...
$(function() {//jQuery入口函数
	...
	$("#saveButton").on("click",function(){
		...
		//ajaxSubmit 发送saveForm表单中的action="/system/slide/save" method="post" 请求
		$("#saveForm").ajaxSubmit({
			success : function(msg) {
				//根据返回对象的信息进行判断
				if (msg.success) {
